...
interface ZodFieldLike {
  ...
  unwrap?: () => ZodFieldLike;
  _def?: { innerType?: ZodFieldLike };
  description?: string;
}
export interface ToolLike {
  name: string;
  description: string;
  schema: { shape?: Record<string, ZodFieldLike>; parse: (value: unknown) => unknown };
...
function unwrap(field: ZodFieldLike): { inner: ZodFieldLike; optional: boolean } {
  let optional = false;
  let current = field;
  while (
    current &&
    ...
    current = typeof current.unwrap === "function"
      ? current.unwrap()
      : (current._def?.innerType as ZodFieldLike);
  }
  return { inner: current, optional };
}

function kindOf(field: ZodFieldLike): Kind {
